...
/**
 * @brief Проверка на нулевое состояние
 * 
 * Нулевое состояние - это состояние 0 (все выключено)
 * В контексте этого модуля просто возвращает true для состояния 0
 */
static bool is_zero_state(uint8_t state) {
    return (state == 0);
}
